...
from src.data.store import review_queue
...
def handle_get_review_queue() -> List[ReviewItem]:
    ...
    return [ReviewItem(**item) for item in review_queue.values()]
...
def handle_submit_review(payload: ReviewSubmission) -> ReviewActionResult:
    ...
    item = review_queue.get(payload.review_id)
    if item is None:
    ...
    del review_queue[payload.review_id]
